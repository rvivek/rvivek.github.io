...
Github's issue tracker is a simple and convenient way to manage a project's internal development process. However, its simplicity can cause unintended consequences. The moment your project gets popular, other people will be using it too. Having people outside of the development team use the same tracker requires special care you might be unprepared for.
...
After a few missteps like this, we evolved an issue etiquette so we can be more helpful to people using the tracker. In the process, I also found that the exact same issue etiquette is extremely helpful to me as a project manager, so I decided to write it down. This list of simple Etiquette Guidelines (*EGs*) can go a very long way to keep users happy.
