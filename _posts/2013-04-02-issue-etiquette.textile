...
Github's issue tracker is a simple and convenient way to manage an internal development process. However, its simplicity can cause unintended consequences -- the moment your project gets popular, other people will be using it too. Having people outside of the development team use the same tracker requires special care,

Here's an example from RethinkDB: https://github.com/rethinkdb/rethinkdb/issues/408#issuecomment-15158650. I talked to everyone internally, determined that there is nothing left to be done, and closed the issue. The thought that others might be using it as an indicator for when an external project fixes their issue didn't even occur to me. After a few missteps like this, we evolved an issue etiquette so we can be more helpful to people using the tracker. In the process, I also found that the exact same issue etiquette is extremely helpful to me as a project manager, so I decided to write it down.
