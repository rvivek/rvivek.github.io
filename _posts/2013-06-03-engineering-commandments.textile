...
Pick the simplest, most concrete possible implementation that solves the customer's problem. Avoid big abstractions -- functions are better than "lenses":http://hackage.haskell.org/package/lens 99.999% of the time. Use simpler language features whenever possible -- an if statement is better than a template specialization. Great abstractions simultaneously feel very powerful in that you can accomplish an enormous number of different tasks with them, and very concrete in that doing any specific task is easy and simple to understand. That's why functions and pipes are better abstractions than monads and "arrows":http://www.haskell.org/arrows/.
...
If you absolutely must do a big rewrite, be tripple vigilent about it. Peer review the initial idea, peer review milestones, be ruthless about cutting complexity and scope. A big rewrite will take at least three times what you've estimated (if you're lucky).
...
As the Klingons say it, "the truth must be won on the battlefield." Sooner or later, some of your team members will strongly disagree with some of these ideas.

Debate everything vigorously, but once the decision is made, make sure _everyone_ is on board. That means pestering people to follow the rules until the process becomes self-perpetuating, confronting people that undermine decisions they disagree with, and, in pathological cases, firing talented engineers who can't buy into the overall engineering culture.

It can be very unpleasant, but it must be done. Remember -- your first responsibility is to the team.