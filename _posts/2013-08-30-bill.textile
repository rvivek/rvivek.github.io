...
bc. git log --pretty=format:"%H|%ae|%at|%s" |     \
...
p. Looks good. I then switched to the here of the occassion himself:

bc. r.table('commits').filter({email: 'bill@rethinkdb.com'}).count()
// 942

During his tenure here, Bill pushed 942 commits. Cool! Let's see what his very first commit was:

bc. r.table('commits').filter({email: 'bill@rethinkdb.com'}).orderBy('date').nth(0)
// {
...
bc. r.table('commits').filter({email: 'bill@rethinkdb.com'}).orderBy(r.desc('date')).nth(0)
// {
...
bc. r.table('commits').filter({email: 'bill@rethinkdb.com'})
...
bc. r.table('commits').filter({email: 'bill@rethinkdb.com'}).filter(r.row('subject').match('fixes')).count()
// 145
...
p. What about some features he added?

bc. r.table('commits').filter({email: 'bill@rethinkdb.com'}).filter(r.row('subject').match('adds')).count()
// 127
...
p. Lots of driver work! Let's look at one random commit that Bill will be remembered by:

bc[js]. r.table('commits').filter({email: 'bill@rethinkdb.com'}).sample(1)
// [
//   {
