...
First, I wrote a shell one-liner to import @git log@ data into RethinkDB:

<pre><code>git log --pretty=format:"%H|%ae|%at|%s" |                \
  python -c "                                            \
...
//   "subject":  "fixing missing subcommand string in log statement \
//                for unrecognized subcommand"
// }
{% endhighlight %}

Looks good. I then switched to the hero of the occassion himself:
...
Ah, that was back when Bill was an intern in 2011. RethinkDB still had a memcached front-end back then (way before we implemented ReQL and Bill's drivers that I'm using to run these queries). I then wondered what his last commit was:
