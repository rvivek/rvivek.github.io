...
<pre><code>git log --pretty=format:"%H|%ae|%at|%s" |     \
  python -c "import rethinkdb as r;           \
             ...
             r.table('commits').update({'date': r.epoch_time(r.row['date'].coerce_to('number')) }).run(durability='soft');"</code></pre>
...
<pre><code class="javascript">r.table('commits').count()
// 20580
...
<pre><code class="javascript">r.table('commits').filter({email: 'bill@rethinkdb.com'}).count()
// 942</code></pre>

During his tenure here, Bill pushed 942 commits. Cool! Let's see what his very first commit was:

<pre><code class="javascript">r.table('commits').filter({email: 'bill@rethinkdb.com'}).orderBy('date').nth(0)
// {
...
<pre><code class="javascript">r.table('commits').filter({email: 'bill@rethinkdb.com'}).orderBy(r.desc('date')).nth(0)
// {
...
<pre><code class="javascript">r.table('commits').filter({email: 'bill@rethinkdb.com'})
...
<pre><code class="javascript">r.table('commits').filter({email: 'bill@rethinkdb.com'}).filter(r.row('subject').match('fixes')).count()
// 145
...
<pre><code class="javascript">r.table('commits').filter({email: 'bill@rethinkdb.com'}).filter(r.row('subject').match('adds')).count()
// 127
...
<pre><code class="javascript">r.table('commits').filter({email: 'bill@rethinkdb.com'}).sample(1)
// [
//   {
