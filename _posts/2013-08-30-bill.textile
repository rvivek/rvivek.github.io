...
<pre><code>git log --pretty=format:"%H|%ae|%at|%s" |     \
  python -c "                                            \
    ...
    r.table('commits').insert([dict(map(lambda x, y: [x, y],                 \
                                        ...
                                        line.split('|', 3))) for line in sys.stdin])
     .run(durability='soft');                            \
    r.table('commits').update({'date': r.epoch_time(r.row['date'].coerce_to('number')) }) \
...
<pre><code class="javascript">r.table('commits')
 .filter({email: 'bill@rethinkdb.com'}).filter(r.row('subject').match('adds')).count()
// 127
