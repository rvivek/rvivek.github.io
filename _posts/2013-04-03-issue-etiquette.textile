...
Here's a real example of a "comment":https://github.com/rethinkdb/rethinkdb/issues/561#issuecomment-16321252 that has poor tone:

bq. This entire thread arguing about the technical possibility of doing this is pointless because @wmrowan's first reply is the right answer.
Even if the conversation didn't end with gotchyas about how wrapping things in parentheses would cause worse confusion, it would still be a bad idea because such a finish to the conversation doesn't mean such gotchyas don't exist. The rest of the thread is people entertaining each other's inability to reason with incomplete information.
Trying to make a friendlier error message is a better solution.
...
bq. I feel really strongly that @wmrowan's first reply is the right answer. It _may_ be technically possible to make this better, but I don't think we should. We already came up with a few gotchyas about how wrapping things in parentheses would confuse people, but even if we didn't, it's very likely that more such gotchayas are lurking under the surface.
I think that trying to make a friendlier error message is a much better solution. What do you think of doing that instead?
